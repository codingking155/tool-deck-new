...
export function Particles({ reduced, theme }) {
  ...
  useEffect(() => {
    ...
    const ps = Array.from({ length: N }, (_, i) => ({
      ...
      vx: (Math.random() - 0.5) * 0.25, vy: (Math.random() - 0.5) * 0.25, r: 0.8 + Math.random() * 1.6,
    ...
    const tick = () => {
      ...
      for (const p of ps) {
        ...
        if (d2 < 12000) { p.x += dx * 0.012; p.y += dy * 0.012; }
        p.x += p.vx; p.y += p.vy;
...
export function CursorGlow({ reduced }) {
  ...
  useEffect(() => {
    ...
    const f = (e) => { el.style.left = e.clientX + "px"; el.style.top = e.clientY + "px"; };
